Generate and install the app icon for /tmp/sidra-style-pulse.
...
/tmp/sidra-style-pulse/app/src/main/res/drawable/icon.png
...
```text
professional 512 by 512 vector app icon, bold high-contrast lettering "SSP" combined with shopping bag, price tag, storefront accent, product silhouettes integrated into the design, solid opaque colored rounded-square canvas using #7B1E3A, clearly rounded app-icon corners with transparent pixels outside the rounded canvas, perfectly centered composition, all shapes complete and uncut, clean flat vector illustration, sharp and fully legible typography, commercial app icon quality
...
After saving icon.png, validate it with `bash /home/codex-agent/codex-app-agent/validate-icon.sh /tmp/sidra-style-pulse/app/src/main/res/drawable/icon.png`.
